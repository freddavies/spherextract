...
RESOURCE_PATH = importlib.resources.files("spherex")
...
def download_cutout_pixels(ra, dec, cutout_size_deg=0.05):
    ...
    custom_mask = ~(
        _BAD_BITS | _BAD_BITS_BKG | _BAD_BITS_MISC
    )  # we actually want all the pixels, and will mask later on
    query = (
...
def cutout_pixels_to_images(pixels, image_tab, ra, dec, cutout_size):
    ...
    all_ra, all_dec = healpy.pix2ang(2**22, all_hp[sort_idx], lonlat=True, nest=True)
...
def _make_psf_detgrid(psf_img, oversamp, cutout_shape, xcut, ycut):
    ...
    hr = psf_img[cy_hr - m // 2 : cy_hr + m // 2, cx_hr - m // 2 : cx_hr + m // 2]
    ...
    canvas[max(0, y0c) : min(Hr, y0c + m), max(0, x0c) : min(Wr, x0c + m)] = hr[
        ys0:ye1, xs0:xe1
    ]
    ...
    shifted = ndimage.shift(
        canvas,
        shift=(dY_det * oversamp, dX_det * oversamp),
        order=3,
        mode="constant",
        prefilter=True,
    )
...
def optimal_extract(
    ...
    def _nan_result(**overrides):
        ...
        base = dict(
            ...
            bkg_npix=0,
            opt_flux_MJysr=None,
            opt_flux_MJysr_err=None,
            opt_flux_uJy=None,
            opt_flux_uJy_err=None,
            opt_snr=None,
            opt_chi2=None,
            opt_dof=None,
            xpix_fulldet=None,
    ...
    # ================================================================
    xctr_items = sorted(
        [(int(k.split("_")[1]), hdr_psf[k]) for k in hdr_psf if k.startswith("XCTR_")]
    )
    yctr_items = sorted(
        [(int(k.split("_")[1]), hdr_psf[k]) for k in hdr_psf if k.startswith("YCTR_")]
    )
    ...
    try:
        if (
            ...
            wv_um = (
                wave[y1, x1] * (x2 - xcut) * (y2 - ycut)
                + wave[y2, x1] * (x2 - xcut) * (ycut - y1)
                + wave[y1, x2] * (xcut - x1) * (y2 - ycut)
                + wave[y2, x2] * (xcut - x1) * (ycut - y1)
            )
            wv_width_um = (
                dwave[y1, x1] * (x2 - xcut) * (y2 - ycut)
                + dwave[y2, x1] * (x2 - xcut) * (ycut - y1)
                + dwave[y1, x2] * (xcut - x1) * (y2 - ycut)
                + dwave[y2, x2] * (xcut - x1) * (ycut - y1)
            )
    except:
        wv_um = np.nan
        wv_width_um = np.nan
...
def _plot_extraction(
    ...
    axs[0].imshow(data, **kw_img)
    axs[0].set_title("Data (bkg-subtracted)")
    axs[1].imshow(model, **kw_img)
    axs[1].set_title("Optimal model  (f·P)")
    axs[2].imshow(resid, **kw_res)
...
def _read_input_file(filename):
    ...
    for delim in [None, " ", "\t", ","]:
        try:
            ...
            continue
    raise ValueError(
        f"Cannot parse {filename}. Expected columns: name  ra  dec  (decimal degrees)."
    )
...
def _build_parser():
    ...
    grp.add_argument(
        "--input", "-i", metavar="FILE", help="Input file with columns: name ra dec"
    )
    grp.add_argument(
        "--ra", type=float, help="Target RA in decimal degrees (single target)"
    )

    p.add_argument(
        "--dec", type=float, help="Target Dec in decimal degrees (required with --ra)"
    )
    p.add_argument(
        "--name",
        type=str,
    ...
    # Output / display
    p.add_argument("--debug", action="store_true", help="Print diagnostic messages")
    p.add_argument(
        "--show-figs", action="store_true", help="Show matplotlib figures interactively"
    )
    p.add_argument("--save-figs", action="store_true", help="Save diagnostic figures")
    p.add_argument(
        "--results-dir",
        default="spherex_results",
...
def main(argv=None):
    ...
    image_tab = pyarrow.parquet.read_table(
        os.path.join(RESOURCE_PATH, "image.parquet")
    )
    # Load in PSF model cubes
    psf_cubes = [
        fits.open(
            os.path.join(
                RESOURCE_PATH / "psf", f"average_psf_D{ii}_spx_cal-psf-v5-2026-082.fits"
            )
        )
        for ii in range(1, 7)
    ]
    ...
    for name, ra, dec in targets:
        ...
        try:
            cutout_pixels = download_cutout_pixels(
                ra=ra, dec=dec, cutout_size_deg=args.cutout_size
            )
        except:
            print(
                "   PixelQuery failed. Trying again with a divide-and-conquer strategy..."
            )
            sleep(5)
            ...
            try:
                ...
                print("   Query 1/5 (0,0)")
                cutout_pixels1 = download_cutout_pixels(
                    ra=ra, dec=dec, cutout_size_deg=size
                )
                print("   Query 2/5 (1,0)")
                cutout_pixels2 = download_cutout_pixels(
                    ra=ra + size / 2, dec=dec, cutout_size_deg=size
                )
                print("   Query 3/5 (-1,0)")
                cutout_pixels3 = download_cutout_pixels(
                    ra=ra - size / 2, dec=dec, cutout_size_deg=size
                )
                print("   Query 4/5 (0,1)")
                cutout_pixels4 = download_cutout_pixels(
                    ra=ra, dec=dec + size / 2, cutout_size_deg=size
                )
                print("   Query 5/5 (0,-1)")
                cutout_pixels5 = download_cutout_pixels(
                    ra=ra, dec=dec - size / 2, cutout_size_deg=size
                )
                cutout_pixels = pyarrow.concat_tables(
                    [
                        cutout_pixels1,
                        cutout_pixels2,
                        cutout_pixels3,
                ...
                print(
                    "   PixelQuery failed yet again. Try reducing the primary cutout size."
                )
                sleep(5)
                failed.append(name)
                continue

        cutout_images = cutout_pixels_to_images(
            cutout_pixels, image_tab, ra, dec, args.cutout_size
        )
